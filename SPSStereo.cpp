...
void SPSStereo::setLabImage(const cv::Mat & leftImage) {
	...
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			cv::Vec3b rgbPixel = leftImage.at<cv::Vec3b>(y, x);
...
void SPSStereo::makeOutputImage(/*png::image<png::gray_pixel_16>*/ cv::Mat & segmentImage, /*png::image<png::gray_pixel_16>*/ cv::Mat& segmentDisparityImage) const {
	...
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			...
			if (estimatedDisparity <= 0.0 || estimatedDisparity > 255.0) {
				segmentDisparityImage.at<uint16_t>(y, x) = 0;
			} else {
				segmentDisparityImage.at<uint16_t>(y, x) = static_cast<uint16_t>(estimatedDisparity*outputDisparityFactor_ + 0.5);
			}
		}
	}
}
